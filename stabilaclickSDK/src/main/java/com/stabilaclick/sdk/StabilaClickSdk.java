...
import com.stabilaclick.sdk.download.DownLoadActivity;
import com.stabilaclick.sdk.sdkinterface.IStabilaClickSdk;
import com.stabilaclick.sdk.utils.AppFrontBackUtils;
import com.stabilaclick.sdk.utils.AppUtils;
import com.stabilaclick.sdk.utils.SignUtils;
...
public class StabilaClickSdk implements IStabilaClickSdk {
    ...
    private static final String ENTER_URI = "stabilaclick://account/enter";
    private IStabilaSDKInterface mStub;
    private static final String TAG = "StabilaClickSdk";
    ...
    private static class SingletonHolder {
        private static final StabilaClickSdk INSTANCE = new StabilaClickSdk();
    }

    private StabilaClickSdk() {
    }

    public static final IStabilaClickSdk getInstance() {
    ...
    private void bindService() {
        ...
        intent.setAction("com.stabilaclick.wallet.StabilaSDKService");
        //android 5.0以后直设置action不能启动相应的服务，需要设置packageName或者Component。
        intent.setPackage("com.stabilaclick.wallet"); //packageName 需要和服务端的一致.
...
//    }
    ...
    @Override
    public double getBalanceStb(String address, boolean isBase58) {
        if (adjustNotEmpty()) {
            try {
                if (mStub != null && mStub.asBinder().isBinderAlive()) {
                    return mStub.getBalanceStb(address, isBase58);
                }
                else {
                    mStub = null;
                    bindService();
    ...
    @Override
    public byte[] createStbTransaction(String fromAddress, String toAddress, double amount) {
        if (adjustNotEmpty()) {
            try {
                if (mStub != null && mStub.asBinder().isBinderAlive()) {
                    return mStub.createStbTransaction(fromAddress, toAddress, amount);
                }
                else {
                    mStub = null;
                    bindService();
    ...
    @Override
    public String createStbTransactionJson(String fromAddress, String toAddress, double amount) {
        if (adjustNotEmpty()) {
            try {
                if (mStub != null && mStub.asBinder().isBinderAlive()) {
                    return mStub.createStbTransactionJson(fromAddress, toAddress, amount);
                }
                else {
                    mStub = null;
                    bindService();
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            mStub = IStabilaSDKInterface.Stub.asInterface(service);
            try {
